...
public class SafeBot extends Player {
	...
	public void play(Turn turn) {
		if (turn.getPhase() == RoundPhase.PRE_FLOP) { // actually it's pre flop
			if (PokerTools.DEBUG) System.out.println("preflop strategy played: " + turn.toString());
		...
		if (turn.getAmountToContinue() - bet > 0) {
			...
			if (PokerTools.DEBUG) System.out.println(turn.toString());
			makeMove(turn, handStrength.getHandStrength(), turn.getAmountToContinue() - bet);
		} else {
			if (PokerTools.DEBUG) System.out.println("custom move of mhamed");
			if (PokerTools.DEBUG) System.out.println(turn.toString());
